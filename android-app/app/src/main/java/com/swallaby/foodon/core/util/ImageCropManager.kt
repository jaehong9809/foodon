...
class ImageCropManager(private val context: Context) {
    ...
    fun loadAndCropImage(
        ...
        val request = ImageRequest.Builder(context).data(imageUrl).target { result ->
            ...
            cropRegions.forEach { position ->
                try {
                    // 비율 값을 실제 픽셀 값으로 변환
                    val xPixel = (position.x * imageWidth).toInt()
                    val yPixel = (position.y * imageHeight).toInt()

                    Log.d(TAG, "Cropping image at x=$xPixel, y=$yPixel")
                    Log.d(TAG, "Image width: $imageWidth, Image height: $imageHeight")
                    ...
                    // 경계 확인
                    if (xPixel < 0 || yPixel < 0 || xPixel + widthPixel > imageWidth || yPixel + heightPixel > imageHeight) {
                        Log.e(
                            TAG,
                            "Crop region out of bounds: position=$position, " + "calculated: x=$xPixel, y=$yPixel, width=$widthPixel, height=$heightPixel"
                        )
                        return@forEach
                    }
                    ...
                    Log.d(
                        TAG,
                        "Cropping image at x=$xPixel, y=$yPixel, " + "width=$widthPixel, height=$heightPixel"
                    )

                    val croppedBitmap = Bitmap.createBitmap(
                        bitmap, xPixel, yPixel, widthPixel, heightPixel
                    )

                    // 캐시에 저장
                    val cacheKey = buildCacheKey(imageUrl, position)
                    imageLoader.memoryCache?.set(
                        MemoryCache.Key(cacheKey), MemoryCache.Value(croppedBitmap)
                    )
    ...
    fun getCroppedImageRequest(imageUrl: String, position: Position): ImageRequest {
        val cacheKey = buildCacheKey(imageUrl, position)
        return ImageRequest.Builder(context).data(imageUrl).memoryCacheKey(cacheKey)
            .diskCacheKey(cacheKey).build()
    }
}